    ...
    run: async(client, message, args, container) => {
        message.channel.send("Searching users...").then(async msg => {
            ...
            const embed = new MessageEmbed()
                .setColor('#B65FCF')
                .setTitle(`Top ${leaderboardinfo.data.leaderboard.length} osu!relax players in the server`)
                .setURL(`https://${botConfig.domain.replace('https://','')}/leaderboard/std/pp/rx`)
                .addFields(
