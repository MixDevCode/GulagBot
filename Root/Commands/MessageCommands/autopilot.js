            ...
            let playercount = await api().get('get_player_info', {
              ...
              .then(async function (playerinfo) {
                ...
                const embed = new MessageEmbed()
                    .setColor('#B65FCF')
                    .setDescription(`**osu!Autopilot status for: [${playerinfo.data.player.info.name}](${botConfig.domain}/u/${playerinfo.data.player.info.id})**`)
                    .setThumbnail(`https://a.mixdev.online/${playerinfo.data.player.info.id}?${Math.floor(Math.random()*9999)}`)
                    .addFields(
                        { name: 'Performance', value: `--- **${playerinfo.data.player.stats[6].pp}pp**\n**Global Rank** #${playerinfo.data.player.stats[6].rank} (:flag_${playerinfo.data.player.info.country}: #${playerinfo.data.player.stats[6].country_rank})\n**Accuracy:** ${Number((playerinfo.data.player.stats[6].acc).toFixed(2))}%\n**Playcount:** ${playerinfo.data.player.stats[6].plays}\n**Level:** ${level(playerinfo.data.player.stats[6].rscore)}`, inline: true },
                        { name: 'Rank', value: `${ranks("SSH")}: ${playerinfo.data.player.stats[6].xh_count}\n${ranks("SS")}: ${playerinfo.data.player.stats[6].x_count}\n${ranks("SH")}: ${playerinfo.data.player.stats[6].sh_count}\n${ranks("S")}: ${playerinfo.data.player.stats[6].s_count}\n${ranks("A")}: ${playerinfo.data.player.stats[6].a_count}`, inline: true },
                    )
