            ...
            .then(function (playerinfo) {
				...
				.then(async function (recentinfo) {
                    ...
                    const embed = new MessageEmbed()
                        .setColor('#B65FCF')
                        .setAuthor({ name: `Most recent osu!Standard play for ${playerinfo.data.player.info.name}`, iconURL: `https://a.mixdev.online/${playerinfo.data.player.info.id}`})
                        .setDescription(`1. ${stars(modsfixed.data[0].difficultyrating,recentinfo.data.scores[0].beatmap.mode)} **[${recentinfo.data.scores[0].beatmap.title}](https://osu.ppy.sh/beatmapsets/${recentinfo.data.scores[0].beatmap.set_id}#osu/${recentinfo.data.scores[0].beatmap.id})** (${Number((parseFloat(modsfixed.data[0].difficultyrating)).toFixed(2))}★) \`${modEnum({mod: recentinfo.data.scores[0].mods}).mod_text}\` • ${score(recentinfo.data.scores[0].score)}\n${ranks(recentinfo.data.scores[0].grade)} *${recentinfo.data.scores[0].beatmap.version}* • **${Number((recentinfo.data.scores[0].pp).toFixed(2))}pp** • x${recentinfo.data.scores[0].max_combo}/${recentinfo.data.scores[0].beatmap.max_combo}\n${Number((recentinfo.data.scores[0].acc).toFixed(2))}% \`[ ${recentinfo.data.scores[0].n300} • ${recentinfo.data.scores[0].n100} • ${recentinfo.data.scores[0].n50} • ${recentinfo.data.scores[0].nmiss} ]\``)
